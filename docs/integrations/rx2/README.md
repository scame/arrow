...
obs
// ObservableKW(observable=io.reactivex.internal.operators.observable.ObservableFromArray@3d64e0b9)
```

```kotlin
val flow = Flowable.fromArray(1, 2, 3, 4, 5).k()
flow
// FlowableKW(flowable=io.reactivex.internal.operators.flowable.FlowableFromArray@35aa848a)
```

```kotlin
val subject = PublishSubject.create<Int>().k()
subject
// ObservableKW(observable=io.reactivex.subjects.PublishSubject@1d87eecc)
```
...
```kotlin
obs.value()
// io.reactivex.internal.operators.observable.ObservableFromArray@3d64e0b9
```

```kotlin
flow.value()
// io.reactivex.internal.operators.flowable.FlowableFromArray@35aa848a
```

```kotlin
subject.value()
// io.reactivex.subjects.PublishSubject@1d87eecc
```
